...
FOLDER=$1
FUZZ_TIME=$2
OUTPUT="output/out${FOLDER}"
CORPUS="input/inp${FOLDER}"
...
if echo "$VAR" | grep -q "error:"; then
  echo "error:"
    ...
    echo "35333241374232453942364639090E000000" > ${CORPUS}/0.txt
    for i in `seq 5`
    do
         RNDTXT=$(cat /dev/urandom | tr -dc '!-~' | fold -w 15 | head -n 1)
         echo $RNDTXT > "${CORPUS}/{i}.txt"
    done

    afl-fuzz -t 50 -V ${FUZZ_TIME} -i ${CORPUS} -o ${OUTPUT} ./${DRIVER}
